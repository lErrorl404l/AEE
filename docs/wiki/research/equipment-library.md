...
## The carried inventory load (the load-carriage addition)

The slots above weigh what the soldier WEARS. What the soldier CARRIES is
a separate walk, `aee_physiology_fnc_getInventoryLoad`:

- the container contents (`items`), one mass per instance
- the assigned slot items (`assignedItems`, plus `hmd` and `binocular`
  when the engine does not report them there): map, compass, watch, GPS,
  radio, NVG, binocular
- the weapon attachments (optic, laser, suppressor, bipod)

The walk skips the worn slots, the carried weapons and the magazines,
because the slot classifiers, `getWeaponLoad` and `getMagazineLoad` already
weigh them. Phase 58 of the docker gate proves the two engine facts the
walk needs: `items` and `assignedItems` do not overlap, and each test item
reaches the walk exactly once.

`aee_physiology_fnc_getItemMass` resolves an item classname to kg. It is
GENERATED by `tools/validation/gen_equipment_data.py` from the research
captures under `data/equipment/sources/` (schema:
`data/equipment/SCHEMA.md`). A capture names a lowercase family keyword that
a classname carries (`pvs14`, `peq15`, `prc152`, `alice`), and the table
holds the median published mass for that family and category, with the
number of published rows behind it. An unknown classname returns 0: it is
unknown, not guessed, and `test_inventory_load.py` fails when a capture
lands without a regenerated resolver. The slot classifiers consult the same
table first, so a published family mass outranks the tier value; armour,
NIR and clo stay with the tier.

The engine `load` command returns 0..1 of a container's capacity, a
fraction and not a mass. An earlier version added that fraction to the
combined load as if it were kg. The walk replaced it, and phase 58 pins the
ratio semantics.

### The derived tier (planned, ADR-001 measure-from-model)

An item with no capture and no family keyword resolves to 0 today. The next
tier measures the item's own model, the principle `fnc_measureBarrel`
already uses (ADR-001). `boundingBoxReal` returns the real dimensions in
metres, so a locally created simple object (`createSimpleObject`) yields an
envelope volume; the mass follows as envelope volume x effective density,
where the density is CALIBRATED per item class against the researched
masses and never assumed. A derived value is grade Derived (ADR-003):
formula and inputs recorded, and it never displaces a found value.
