...
**What happened.** `~/.config/opencode/rules/secret-scan.py` reports findings
on two generated vehicle files, `data/vehicle/classes.json` and
`data/vehicle/coverage.json`. Each finding names the `assigned-literal` rule at
a `"token": "..."` key whose engine class string is 20 characters or longer.
The tool reports the path, the line and the value length, never the value. The
class inventory is generated by
`tools/validation/gen_vehicle_class_inventory.py`. The coverage table is
generated by `tools/validation/gen_vehicle_coverage.py` and carries the same
`"token"` key for every inventory token, so it triggers the same rule.
...
**What prevents recurrence.** Both files are generated data, so a hand edit is
not an option. This change records the false positive instead. The next change
to the scanner should add a per-path or per-rule exemption, for example a
generated-data allowlist keyed by path, so a real credential in the same file
is still caught. The vehicle gate keeps the inventory and the coverage table
fresh.
...
## GAP-031: two arithmetic bugs in a research calculation

Status: Open

**What happened.** While deriving the condensation onset angle for issue 217, the
analysis carried two defects. First, relative humidity was passed as a percent
into an expression that requires a fraction, so every supersaturation value was
one hundred times too large. That produced the physically impossible result
that a stream with no turn relative to the ambient had a supersaturation of 50.
Second, where the local temperature fell below the minus 80 C validity floor of
the Alduchov and Eskridge over-ice fit, the code returned no value, and the
bisection read that as "the target has not been reached". That state only occurs
at large turn, so the search ran to its bracket limit and reported 45 degrees
for every case at Mach 1.9 and above. That number was a search artefact and
looked like a physical result.

Both defects were caught before anything reached the issue tracker. The
correction is recorded at
<https://github.com/lErrorl404l/AEE/issues/217#issuecomment-5849071779>.

**What went wrong.** No invariant was asserted on either quantity. A
supersaturation of an unturned stream must equal the ambient relative humidity
as a fraction, and that identity would have caught the unit error immediately. A
monotonic search must have its endpoint values checked before the bracket runs,
and no such check existed. The same defect class as GAP-029, in which a
formula was published without being evaluated at the boundary of its regime.

**Why.** The rule was unapplied, not absent. Rule 3 of the global operating
rules requires verification before stating a fact, and the repo has an existing
mirror-test pattern in `tools/tests/` where a Python model asserts against the
SQF. The research calculation sat outside that harness entirely, so it had no
assertions and no gate. Nothing in the workflow forces an exploratory script to
carry a self-check.

**What prevents recurrence.** A derived number bound for a tracker or a data
file must carry its invariants in the same script. For a supersaturation field
the identity is that zero turn reproduces the ambient humidity. For a bisection
the endpoints must be printed and checked for sign change before the bracket is
trusted. Where a source fit has a stated validity range, the calculation must
refuse outside it rather than extrapolate, and the refusal must be counted as
past the target when the quantity is monotonic. The tracker comment for this
issue now records both bugs, the corrected table, and the assertion that caught
each one.
