...
private _overcastS = ([] call EFUNC(core,getSmoothedWeather)) select 1;
...
private _lux = missionNamespace getVariable [QEGVAR(core,illuminanceLux), 0.001];
...
private _blowoutNow = 0;
{
    ...
    if (_isBright) then {
        ...
        if (_clear && _ang < _releaseHalf) then {
            ...
            private _fogS2 = ([] call EFUNC(core,getSmoothedWeather)) select 2;
...
private _focusEye = [_player] call EFUNC(core,getEyeState);
