...
private _blurScale   = missionNamespace getVariable [QGVAR(severeWeatherBlur), 1.0];
...
if (_sandstorm > 0 || _dustDevil > 0) then {
    ...
    missionNamespace setVariable [QEGVAR(optics,severeWeatherBlur), (_windSpeed / 10) * _blurScale];
    } else {
